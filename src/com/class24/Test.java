package com.class24;

//testing inheritance

public class Test {
	public static void main(String[] args) {
		Child1 child1=new Child1();
		
		System.out.println(Child1.race);//<---static variable. just use class
		
		System.out.println(child1.hairColor);
		System.out.println(child1.eyeColor);
		child1.sing();
		child1.code();
		
		Parent parent=new Parent();
		System.out.println(parent.hairColor);
		System.out.println(parent.eyeColor);
		System.out.println(Parent.race);//<---static variable. just use class
		parent.sing();
		
		
		Child2 child2=new Child2();
		System.out.println(Child2.race);//<---static variable. just use class
		System.out.println(child2.eyeColor);
		System.out.println(child2.hairColor);
		child2.draw();
		child2.sing();
	}
	
}
